...
chance.mixin({
  user: function () {
    let country = chance.pickone(chance.countries);
    let game = chance.pickone(chance.games);
    let montlyWinnings = Array.from({ length: 12 }).map(() =>
      chance.floating({ min: 100, max: 1000, fixed: 3 })
    );
    let toDollar = (value) => `$${value}`;
    ...
    return {
      ...
      totalWinnings: toDollar(
        montlyWinnings.reduce((month, nextMonth) => month + nextMonth, 0).toFixed(3)
      ),
...
const getUsers = (usersNumber = 20) => {
  return Array.from({ length: usersNumber }).map((item, idx) => chance.user());
};
